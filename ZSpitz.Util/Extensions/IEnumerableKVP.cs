...
namespace ZSpitz.Util {
    public static class IEnumerableKVPExtensions {
        public static void AddRangeTo<TKey, TValue>(this IEnumerable<KeyValuePair<TKey, TValue>> toAdd, Dictionary<TKey, TValue> dict) where TKey : notnull => toAdd.ForEach(kvp => dict.Add(kvp.Key, kvp.Value));
        ...
        public static Dictionary<TKey, TValue> ToDictionary<TKey, TValue>(this IEnumerable<KeyValuePair<TKey, TValue>> src) where TKey : notnull => src.ToDictionary(x => x.Key, x => x.Value);
    }
}
